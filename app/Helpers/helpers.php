...
class Helper
{
	...
	const SUBSCRIPTION_DEFAULT = 'default';
    ...
    public static function applClasses()
    {
        ...
        $allOptions = [
            'mainLayoutType' => array('vertical', 'horizontal', 'subscriber'),
    ...
    public static function humanFriendlyText($text)
    {
        ...
        return ucwords(implode(" ", $textArr));
    }

    public static function tooltipInfo($info, $icon = 'help-circle')
    {
        $text = "<span class='cursor-pointer' data-toggle='tooltip' title='$info'><i data-feather='" . $icon . "'></i></span>";
        return $text;
    }

    public static function tooltip($long, $len = 25)
    {
        $short = Str::limit($long, $len, "...");
        $long = str_replace("'", "&#39;", $long);
        $long = str_replace('"', "&#34;", $long);
        return "<span data-toggle='tooltip' title='{$long}'>{$short}</span>";
    }

    public static function isAcademicAddress($email)
    {
        $lastThreeChars = substr($email, strlen($email) - 3, strlen($email));
        return strtolower($lastThreeChars) == "edu";
    }
